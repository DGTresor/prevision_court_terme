...
author: Louise Phung (DG Trésor/SPMAE/Prev3)
date: 18/07/2023
...
# TODO: improve graphs display in html output document
...
source("./regressions_and_previsions.R", encoding = "utf-8", chdir = TRUE)
source("./data_preparator.R", encoding = "utf-8")
source("./data_importator.R", encoding = "utf-8")
source("./graph_plotting/general_graph_functions.R", encoding = "utf-8", chdir = TRUE)
...
# So, always put LOAD_EXISTING_DATA_FOR_HEURISTIQUE to TRUE.
# Note: loading existing data enables to reproduce exactly the results from the Document de Travail
LOAD_EXISTING_DATA_FOR_HEURISTIQUE <- TRUE

```

Ce fichier RMarkdown reproduit les résultats de la partie 4 du Document de Travail.
...
load("../data_doc_travail/data_doc_travail_2023-07-18.RData")
...
```{r figure_1, echo = FALSE, fig.dim = c(9, 7)}
...
suppressMessages(                                                    # suppressMessages() enables to remove the messages related to the modification of the scales.
  compare_insee_pmi_bdf_for_one_index_graph(graph_name = "pib_var_trim_insee_pmi",
                                            ...
                                            title = "Variation trimestrielle du PIB et \nindicateurs synthétiques centrés-réduits",  # Note: the \n symbol enables to return to the line
    ...
    labs(subtitle = "Ecart à la moyenne en point d'écart-type (à gauche pour les indices) \net pourcentage de variation (à droite pour le PIB)") +
    guides(color = guide_legend(nrow = 2))  # put the legend on 2 rows
)
...
suppressMessages(                                                    # suppressMessages() enables to remove the messages related to the modification of the scales.
  compare_insee_pmi_bdf_for_one_index_graph(graph_name = "pib_ga_insee_pmi_bdf",
                                            ...
                                            title = "Glissement annuel du PIB et \nindicateurs synthétiques trimestriels moyens centrés-réduits",
...
```{r figure_2, echo = FALSE, , fig.dim = c(9, 5)}
...
ggplot(data_graph_evol_PIB %>% filter(dimension == "var1_PIB")) +
  ...
  labs(title = "Variation trimestrielle du PIB trimestriel français",
       subtitle = "Période : T2 1998 - T4 2019",
...
```{r tableau_corr_fenetre_glissante_en_niveau, echo = FALSE, fig.dim = c(9, 5)}
...
```{r tableau_regression_with_rolling_window, echo = FALSE, fig.dim = c(9, 5)}
...
ggplot(rolling_regressions_coef) +
  ...
  geom_line() +
  labs(title = paste("Croissance associée à une hausse de 4 pts des climats Insee et BdF \net de 2 pts du PMI
...
Pour le travail à Prev3 spécifiquement afin d'améliorer l'interprétation des enquêtes de conjoncture dans nos réactions

```{r calcul_heuristiques_tous_soldes_fenetre_roulante, echo = FALSE, fig.dim = c(9, 5)}

if (LOAD_EXISTING_DATA_FOR_HEURISTIQUE) {
  load("../data_doc_travail/data_doc_travail_heuristiques_data_20230703.RData")
} else {
  load("../data_doc_travail/revised_pib_2023-01-01PE.RData")
  heuristiques_survey_data <- load_data_for_nowcasting(file.path(DGTRESOR_S_SERVER_PATH, "Synthèse/3_etudes_et_travaux/2023_Document de travail enquêtes de conjoncture et nowcasting en temps réel/donnees/donnees_pour_nowcasting_en_temps_reel_backup.xlsx"))

  # prepare data  -----------
  pib_data <- revised_pib %>%
    get_variation_for(variation_type = "growth_rate", keep_prefix = TRUE)

  va_services_data <- csv_post_19T2RD_national_accounting_loader(file_path = file.path(NATIONAL_ACCOUNTING_DATA_FOLDER_BASE2014, "23T1PE"),
                                                                 folder_name = "23T1PE",
                                                                 file_name = "cprvolch",
                                                                 dimensions_list = list("default" = c("va_services_marchands_non_agri" = "TD.B1_SMNA_7CH")),
                                                                 dimensions_list_name = "default") %>%
    get_variation_for(variation_type = "growth_rate", keep_prefix = TRUE)

  va_industrie_manuf_data <- csv_post_19T2RD_national_accounting_loader(file_path = file.path(NATIONAL_ACCOUNTING_DATA_FOLDER_BASE2014, "23T1PE"),
                                                                        folder_name = "23T1PE",
                                                                        file_name = "cprvolch",
                                                                        dimensions_list = list("default" = c("va_indus_manuf" = "TD.B1_DIM_7CH")),
                                                                        dimensions_list_name = "default") %>%
    get_variation_for(variation_type = "growth_rate", keep_prefix = TRUE)

  va_construction_data <- csv_post_19T2RD_national_accounting_loader(file_path = file.path(NATIONAL_ACCOUNTING_DATA_FOLDER_BASE2014, "23T1PE"),
                                                                     folder_name = "23T1PE",
                                                                     file_name = "cprvolch",
                                                                     dimensions_list = list("default" = c("va_construction" = "TD.B1_A17FZ_7CH")),
                                                                     dimensions_list_name = "default") %>%
    get_variation_for(variation_type = "growth_rate", keep_prefix = TRUE)

  heuristiques_survey_data <- heuristiques_survey_data %>%
    month_to_quarter(transformation_type = "mean")

  heuristiques_data <- pib_data %>%
    dplyr::bind_rows(va_services_data) %>%
    dplyr::bind_rows(va_industrie_manuf_data) %>%
    dplyr::bind_rows(va_construction_data) %>%
    dplyr::bind_rows(heuristiques_survey_data) %>%
    convert_to_wide_format()

  save(heuristiques_data, file = paste0("../data_doc_travail/data_doc_travail_heuristiques_data_", lubridate::today(), ".RData"))
}

subset_heuristiques_data <- heuristiques_data %>%
  dplyr::select(-matches("construction")) %>%
  dplyr::filter(date >= lubridate::ymd("1998-04-01") & date <= lubridate::ymd("2019-10-10")) %>%
  dplyr::filter(!(date %in% outliers_var1_pib_robust)) %>%
  dplyr::arrange(date)
...
subset_heuristiques_data_seuil <- subset_heuristiques_data %>%
  dplyr::mutate(across(matches("(insee)|(bdf)"), ~(. - 100))) %>%
  dplyr::mutate(across(matches("pmi"), ~(. - 50)))

# Régressions au seuil pour calcul heuristique - avec période roulante - pour le PIB
heuristiques_pib <- create_summary_for_several_simple_regressions(y_var = "var1_PIB",
                                                                  list_x_var = colnames(subset_heuristiques_data_seuil)[!(colnames(subset_heuristiques_data_seuil) %in% c("date", "var1_PIB", "var1_B1_SMNA", "var1_B1_DIM", "var1_B1_FZ"))],
                                                                  reg_data = subset_heuristiques_data_seuil,
                                                                  window_size = REG_WINDOW_SIZE) %>%
  dplyr::mutate(var_trim_pib_pour_indice_au_seuil = constant * 100,
                valeur_indice_pour_var_trim_pib_nulle = case_when(
                  stringr::str_detect(dimension, "pmi") ~ 50,
...
tableau_heuristiques_pib <- heuristiques_pib %>%
  dplyr::group_by(dimension) %>%
  dplyr::summarise(coefficient = mean(coefficient),
                   var_trim_pib_pour_indice_au_seuil = mean(var_trim_pib_pour_indice_au_seuil),
                   valeur_indice_pour_var_trim_pib_nulle = round(mean(valeur_indice_pour_var_trim_pib_nulle), 0))

tableau_heuristiques_pib

# Régressions au seuil pour calcul heuristique - avec période roulante - pour la VA dans les services marchands non agricoles
heuristiques_va_services <- create_summary_for_several_simple_regressions(y_var = "var1_B1_SMNA",
                                                                          list_x_var = c("qt_bdf_services", "qt_insee_services", "qt_pmi_services"),
                                                                          reg_data = subset_heuristiques_data_seuil,
                                                                          window_size = REG_WINDOW_SIZE) %>%
  dplyr::mutate(var_trim_va_services_pour_indice_au_seuil = constant * 100,
                valeur_indice_pour_var_trim_va_services_nulle = case_when(
                  stringr::str_detect(dimension, "pmi") ~ 50,
                  TRUE ~ 100
                )) %>%
  dplyr::mutate(valeur_indice_pour_var_trim_va_services_nulle = round(valeur_indice_pour_var_trim_va_services_nulle - (constant / coefficient), 0)) %>%
  select(date, dimension, coefficient, var_trim_va_services_pour_indice_au_seuil, valeur_indice_pour_var_trim_va_services_nulle, adjusted_r_squared, rmse, mae)


tableau_heuristiques_va_services <- heuristiques_va_services %>%
  dplyr::group_by(dimension) %>%
  dplyr::summarise(coefficient = mean(coefficient),
                   var_trim_va_services_pour_indice_au_seuil = mean(var_trim_va_services_pour_indice_au_seuil),
                   valeur_indice_pour_var_trim_va_services_nulle = round(mean(valeur_indice_pour_var_trim_va_services_nulle), 0))

tableau_heuristiques_va_services


# Régressions au seuil pour calcul heuristique - SANS période roulante - pour la VA dans l'industrie manufacturière
heuristiques_va_indus <- create_summary_for_several_simple_regressions(y_var = "var1_B1_DIM",
                                                                       list_x_var = c("qt_bdf_industrie", "qt_insee_industrie", "qt_pmi_industrie"),
                                                                       reg_data = subset_heuristiques_data_seuil,
                                                                       window_size = nrow(subset_heuristiques_data)) %>% # Note: on a des valeurs aberrantes si on utilise des fenêtres roulantes [sûrement des outliers à nettoyer pour l'industrie spécifiquement]
  dplyr::mutate(var_trim_va_indus_pour_indice_au_seuil = constant * 100,
                valeur_indice_pour_var_trim_va_indus_nulle = case_when(
                  stringr::str_detect(dimension, "pmi") ~ 50,
                  TRUE ~ 100
                )) %>%
  dplyr::mutate(valeur_indice_pour_var_trim_va_indus_nulle = round(valeur_indice_pour_var_trim_va_indus_nulle - (constant / coefficient), 0)) %>%
  select(date, dimension, coefficient, var_trim_va_indus_pour_indice_au_seuil, valeur_indice_pour_var_trim_va_indus_nulle, adjusted_r_squared, rmse, mae)


tableau_heuristiques_va_indus <- heuristiques_va_indus %>%
  dplyr::group_by(dimension) %>%
  dplyr::summarise(coefficient = mean(coefficient),
                   var_trim_va_indus_pour_indice_au_seuil = mean(var_trim_va_indus_pour_indice_au_seuil),
                   valeur_indice_pour_var_trim_va_indus_nulle = round(mean(valeur_indice_pour_var_trim_va_indus_nulle), 0))

tableau_heuristiques_va_indus


# Plotting évolution heuristiques - pour PIB --------------------------------------------------------------------------
ggplot(heuristiques_pib) +
  aes(x = date, y = var_trim_pib_pour_indice_au_seuil, color = dimension) +
  geom_line() +
  labs(title = paste("Estimation roulante sur", REG_WINDOW_SIZE / 4, "ans de la croissance trimestrielle du PIB compatible \navec un climat constant à son seuil"))


rolling_regressions_coef <- heuristiques_pib %>%
  dplyr::mutate(coefficient = case_when(
    dimension %in% c("bdf_global_m3", "insee_global_m3", "lead_insee_global_m1") ~ coefficient * 4,
    dimension == "pmi_composite_m3" ~ coefficient * 2,
    TRUE ~ coefficient
...
# Préparation pour exportation -----------------------------------------------------------------------------------------
data_for_excel_heuristiques_pib <- tableau_heuristiques_pib
data_for_excel_heuristiques_va_services <- tableau_heuristiques_va_services
data_for_excel_heuristiques_va_indus <- tableau_heuristiques_va_indus
...
writexl::write_xlsx(list("data_figure_1a" = data_for_excel_figure_1a,
                         ...
                         "annexe_tableau_heuristiques_1" = data_for_excel_heuristiques_pib,
                         "annexe_tableau_heuristiques_2" = data_for_excel_heuristiques_va_services,
                         "annexe_tableau_heuristiques_3" = data_for_excel_heuristiques_va_indus),
