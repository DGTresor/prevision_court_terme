...
# We use data from base2005 onward; the exclusion of data previous to base2005 is dealt in the function: get_production_data_files()

# functions ------------------------------------------------------------------------------------------------------------
# todo: check duplication with construct_nonrevised_ipi_from_scratch()
construct_nonrevised_production_from_scratch <- function(files_list, file_type2files_list, number_previous_values = 0) {
  df <- get_empty_dataframe(number_previous_values)
  # column_to_join_by <- get_column_names_to_join_by()

  loader_provider <- get_loader_provider(data_source = "production")
...
get_production_data_files <- function(production_folder, estimation_type) {
  ...
  production_data_folders <- list.dirs(production_folder, recursive = TRUE, full.names = TRUE)
  ...
  production_data_folders <- production_data_folders[sort(names(production_data_folders))]

  return(production_data_folders)
}
