...
transfo_survey_data <- survey_data %>%
  ...
  mutate(sum_climat = pmi_climat_m1 + pmi_climat_m2 + pmi_climat_m3,
         ...
         sum_new_commandes = pmi_new_commandes_m1 +
           pmi_new_commandes_m2 +
           pmi_new_commandes_m3) %>%
...
for (i in start_from:(nrow(data_final) - 1)) {
  if (!(i %in% c(37, 38, 39))) { # do not take dates for 2020T1, 2020T2, 2020T3 CF. i +1 dans la formule => concerne les trimestres 2020-T2-T4
    ...
    model <- lm(var1_production ~ var1_acquis_m1 + sum_prod_passee_growthrate, data = train_set)
...
start_date <- data_final$date[start_from + 1]
final_date <- max(data_final$date)
test <- test %>%
  mutate(date = seq(from = start_date, to = final_date, by = "3 months")) %>%
...
ggplot(test2, aes(x = quarter, y = value, color = dimension)) +
  geom_line() +
  labs(title = "France : prévision de la production dans l'industrie manufacturière",
       subtitle = "Variation trimestrielle",
       caption = "Derniers points : RD 2022T3, IPI d'octobre et PMI de décembre, \nCalculs DG Trésor") +
  ...
  scale_y_continuous(breaks = c(-0.2, -0.15, -0.10, -0.05, 0, 0.05, 0.1, 0.15, 0.2),
