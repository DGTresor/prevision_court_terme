...
## Notion de nowcasting

* donner une définition
* expliquer comment s'organise le dataframe -> les dates représentent les observations d'obtention des données
...
De 2003 à 2019, la production manufacturière renoue avec un sentier de croissance positif, jusqu'à la crise de la Covid-19.
...
Nous allons nous intéresser à la production industrielle de 2011 à maintenant en utilisant les données non-révisées (la PE) que nous allons prédire.
...
Le test de Phillips-Perron indique qu'on rejette l'hypothèse de non-stationnarité (pour les lags 3 et 9) et le KPSS test indique qu'on ne peut pas rejeter l'hypothèse de stationnarité (pour les lags 3 et 9), ce qui semble indiquer que le taux de croissance de la production manufacturière est stationnaire sur la période 2011-2022.
...
transfo_nonrevised_ipi <- nonrevised_ipi_cz_corrected %>%
  ...
  dplyr::filter(date >= ymd("2011-04-01") & date <= date_max) %>%
...
On notera que les corrélations inscrites ici sont pour les IPI trimestrialisés complets (i.e. quand on a les trois mois d’IPI), donc les corrélations seront plus faibles quand on n’aura que le mois 1 ou mois 2 du trimestre, et encore plus faible au mois 0.
